const PUZZLE_DIFFICULTY = 4; //размер сетки ячейки
  ...
  let ctx, canvas, img, pieces, mainWidth, mainHeight, pieceWidth, pieceHeight, currentPiece, currentDroppedPiece, mouse;
  let count = 0, maxCount = 0, gamesCount = 0;

  function init() {
   img = new Image();
   img.src = "mediumImage/medium.jpg";
  ...
  function initPuzzle(){
   ...
   if (!gamesCount) createTitle("Давай сыграем?!"); 
   buildPieces();
  }
  ...
  function gameOver(){
   ...
   createTitle ('Ты собрала пазл за: ' + count + ' ходов, ' + (count <= maxCount ? 'и это твой новый рекорд! ' : 'а твой рекорд ' + maxCount) + '... сыграем ещё?');
